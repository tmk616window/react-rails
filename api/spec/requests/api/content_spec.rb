  ...
  describe 'PATCH #update' do
    ...
    context '正常に通るか' do
        it "期待通りに編集さてれているか" do              
          patch "/api/contents/#{content.id}", headers: headers, params: { content: {title: "title" ,text: 'text', task_id: task.id}}
          content.reload
    ...
    context 'paramsが空' do
      it "editをレンダリングする" do
        patch "/api/contents/#{content.id}", headers: headers, params: { content: {title: nil,text: nil, task_id: task.id}}
        content.reload
        response_json = JSON.parse(response.body)
