...
RSpec.describe 'Comment', type: :request do
  ...
  describe 'create' do
    context '正常に動いている' do
      it 'taskをリダイレクトする' do
        expect{
          post '/api/comments', headers: headers, params: { comment: {text: 'test', task_id: task.id, user_id: user.id}}
  ...
  describe 'PATCH #update' do
    ...
    context '正常に通るか' do
        it "期待通りに編集さてれているか" do              
          patch "/api/comments/#{comment.id}", headers: headers, params: { comment: {text: 'comment', task_id: task.id, user_id: user.id}}
          comment.reload
    ...
    context '失敗する' do
      it "editをレンダリングする" do
        patch "/api/comments/#{comment.id}", headers: headers, params: { comment: {text: nil, task_id: task.id, user_id: user.id}}
        comment.reload
        response_json = JSON.parse(response.body)
