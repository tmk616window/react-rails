...
const EditTaskDetails:React.FC<EditTaskProps> = ({task, contents, setTask, setContents, setEdit, taskId}) => {
  ...
  const patchContent = async () => {
    ...
    for (const content of contents) {
      if(content.id) {
        const a = (await updateContent(content['title'], content['text'], taskId, content.id)).data.content
        c.push(a)
      } else {
        const b = (await createContent(content['title'], content['text'], taskId)).data.content
        c.push(b)
      }
    }
    return c
